...
public class FileController {
     ...
     */
    public FileController(DataController dataController) {
    ...
    protected void readFromFile(final String path) throws IOException {
        ...
        Matcher matcher = Pattern.compile("(?<=<city>)[\\S\\s]*?(?=</city>)",
                Pattern.CASE_INSENSITIVE).matcher(xmlString);
        ...
        for (String i : list) {
            try {
                ...
                dataController.getCommandController().getLogger().log(Level.WARNING,
                        "Ошибка считывания в файле: ошибка аргументов в этом городе: " +
                                e.getMessage() + "\nГород был пропущен");
            } catch (IncorrectValueException e) {
                dataController.getCommandController().getLogger().log(Level.WARNING,
                        "Ошибка считывания в файле: некорректное значение: " +
                                e.getMessage() + "\nГород был пропущен");
            } catch (NullValueException e) {
                e.printStackTrace();
            } catch (NotUniqueIDException e) {
                dataController.getCommandController().getLogger().log(Level.WARNING,
                        "Ошибка считывания в файле: значение ID не уникально. Город был пропущен");
            }
        }
    }
    ...
    private String getCityXmlString(final City city) {
        ...
        xmlString.append("<coordinates>\n<x>").append(city.getCoordinates().getX()).append("</x>\n<y>")
                .append(city.getCoordinates().getY()).append("</y>\n</coordinates>\n");
        ...
        xmlString.append("<meters_above_sea_level>").append(city.getMetersAboveSeaLevel())
                .append("</meters_above_sea_level>\n");
        ...
        xmlString.append("<governor>\n<age>").append(city.getGovernor().getAge()).append("</age>\n<birthday>")
                .append(city.getGovernor().getBirthday()).append("</birthday>\n</governor>\n");
    ...
    private City parseCity(final String xmlString)
            throws SoManyArgumentsException, EmptyValueException, IncorrectValueException,
            NullValueException, NotUniqueIDException {
        ...
        if (!tempStr.equals("")) {
            ...
            if (city.getClimateString().equals(""))
                dataController.getCommandController().getLogger().log(Level.INFO,
                        "Значение аргумента climate некорректно. Поле было пропущено.");
        }
        ...
        if (!tempStr.equals("")) {
            ...
            if (city.getGovernmentString().equals(""))
                dataController.getCommandController().getLogger().log(Level.INFO,
                        "Значение аргумента government некорректно. Поле было пропущено.");
        }
